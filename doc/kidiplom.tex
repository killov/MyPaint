...
Pokud na objekt klikneme, přepne se do mezistavu, který se nazývá přesouvání. Při přesouvání získáme od grafického objektu bod, je jedno jaký, jen je potřeba, aby grafický objekt věděl, o který bod jde a pamatoval si ho. Grafický objekt bude tento bod potřebovat při výpočtu souřadnic na které se má přesunout. Při pohybu myší třída \kiinlinecode{csharp}{!}{DrawControl} informuje grafický objekt o přesunu a společně s touto informací mu předá bod, který vznikne tak, že k tomu původnímu bodu přičte vektor posunu.

Při odkliknutí myší je přesouvání ukončeno a \kiinlinecode{csharp}{!}{DrawControl} se přepne nazpět do stavu editace. Označený objekt lze kromě přesouvání dále upravovat. Můžeme měnit jeho rozměry díky bodům, které grafický objekt zobrazuje.

Pokud klikneme mimo editovaný objekt, tak se \kiinlinecode{csharp}{!}{DrawControl} přepne nazpět do stavu na začátku kreslení a ihned můžeme kreslit nový objekt.

\subsection{Třída Layer}

Třída \kiinlinecode{csharp}{!}{Layer} reprezentuje vrstvu a uchovává v sobě její název, barvu pozadí, seznam grafických objektů, které obsahuje, informace o tom jestli je vrstva viditelná a také WPF element \kiinlinecode{csharp}{!}{Canvas}, do kterého přidává elementy z grafických objektů. 

Třída implementuje rozhraní \kiinlinecode{csharp}{!}{INotifyPropertyChanged}, aby byl možný obousměrný data-binding. Informuje tak GUI o změnách, aby bylo jasné, že ve vrstvě nastala změna a je potřeba okno překreslit. GUI se pak překreslí podle dat z vrstvy.
...
Pro vytvoření vlastního grafického tvaru je potřeba naprogramovat třídu. Třída musí dědit z třídy \kiinlinecode{csharp}{!}{Shape} a implementovat následující metody:
...
\end{itemize}
Tato metoda je volána pokud uživatel mění u objektu tloušťku. V argumentu dostaneme novou tloušťku. Pokud náš nový grafický objekt tloušťku podporuje a chceme akci o změně tloušťky uložit do historie, pak vrátíme hodnotu \kiinlinecode{csharp}{!}{true}, v opačné případě vrátíme hodnotu \kiinlinecode{csharp}{!}{false}.
...
Vytvořený element následně přidáme do \kiinlinecode{csharp}{!}{Canvas}u, který získáme z argumentu.

\begin{itemize}
\item \kiinlinecode{csharp}{!}{void ChangeZoom()}
\end{itemize}

Volá se v momentě, kdy je změněno měřítko zobrazení plátna. Z \kiinlinecode{csharp}{!}{File.RevScale} je možno získat poměr zvětšení tak, aby byl grafický objekt při změně přiblížení pořád stejně velký. \kiinlinecode{csharp}{!}{File.RevScale} se hodí pro zachování velikosti. Můžeme ho využít například pro zachování velikostí ovládacích bodů objektů. Samotný \kiinlinecode{csharp}{!}{File.RevScale} jde přímo vložit do vlastnosti WPF elemetu tak, aby měnil velikost sám. Příklad zdrojový kód č. \ref{kod:revscale}).
...
Každý grafický objekt by měl mít k dispozici editační prvky, které se zobrazí v momentě, kdy je objekt editován. Editační prvky jsou WPF elementy s naprogramovanými událostmi, které vkládáme při editace do vrchního \kiinlinecode{csharp}{!}{Canvas}u, aby je uživatel viděl před objekty a mohl nimi posouvat.
...
Jak jsem již zmínil, pro ovládací prvky typu malých elips se kterým jde posouvat, vznikla speciální třída pod názvem \kiinlinecode{csharp}{!}{MovePoint}.

K založení instance je zapotřebí počátečního bodu, instanci grafického objektu, horního \kiinlinecode{csharp}{!}{Canvas}u a delegátu \kiinlinecode{csharp}{!}{void MoveDelegate(Point b)}. Delegát je funkce, kterou můžeme ukládat do proměnných a předávat ji jako argumenty funkcí. Náš delegát se zavolá při každé změně pozice a v argumentu dostane bod místa, kam se má přesunout.

Pokud chceme editační prvek zobrazit, zavoláme metodu \kiinlinecode{csharp}{!}{Show()}. V opačném případě zavoláme metodu \kiinlinecode{csharp}{!}{Hide()}.
...
Slouží k editování obdélníkových tvarů (obdélník, elipsa, obrázek). Na čtyři krajní body používá třídu \kiinlinecode{csharp}{!}{MovePoint}. Pokud je aktivní, tak zobrazuje obdélník přerušovanou čárou. Při podržení klávesy shift a přesouvání jednoho z krajních bodů, drží poměr stran. Tato funkce se hodí například při zmenšování nebo zvětšování obrázků.

\subsection{Třída HistoryControl}

Každý soubor má svoji historii všech změn, které při práci v souboru provedeme. O tuto historii se stará třída \kiinlinecode{csharp}{!}{HistoryControl}. Každá třída \kiinlinecode{csharp}{!}{FileControl} si při vytvoření instance vytvoří svou vlastní instanci třídy \kiinlinecode{csharp}{!}{HistoryControl}.
...
Jednotlivé akce jsou reprezentovány instancemi tříd implementujících rozhraní \kiinlinecode{csharp}{!}{IHistoryNode}.
...
\end{itemize}
Testuje, jestli se od posledního zavolání \kiinlinecode{csharp}{!}{SetNotChange()} nebo od vytvoření zapnutí historie tato historie nějak změnila. Používá se k testu, jestli je potřeba soubor uložit. Aplikace tento test používá k tomu, abychom si omylem nezavřeli aplikaci bez uložení.
...
Na test kompatibility dvou akcí \kiinlinecode{csharp}{!}{IHistoryNodeSkipped} slouží metoda \kiinlinecode{csharp}{!}{bool Optimal(IHistoryNodeSkipped node)}, která vrací pravdivostní hodnotu a určuje, zda jsou kompatibilní.
...
\bibitem{cs} Wikipedia: Otevřená Encyklopedie: C Sharp [online]. 2019-07-29. [cit. 2019-07-29]. Dostupné z: \url{https://cs.wikipedia.org/wiki/C\_Sharp}

\bibitem{wpf} Wikipedia: Otevřená Encyklopedie: Windows Presentation Foundation [online]. 2019-07-29. [cit. 2019-07-29]. Dostupné z: \url{https://cs.wikipedia.org/wiki/Windows\_Presentation\_Foundation}

\bibitem{xaml} Wikipedia: Otevřená Encyklopedie: Extensible Application Markup Language [online]. 2019-07-29. [cit. 2019-07-29]. Dostupné z: \url{https://cs.wikipedia.org/wiki/Extensible\_Application\_Markup\_Language}

\bibitem{json} Wikipedia: Otevřená Encyklopedie: JavaScript Object Notation [online]. 2019-07-29. [cit. 2019-07-29]. Dostupné z: \url{https://cs.wikipedia.org/wiki/JavaScript\_Object\_Notation}

\bibitem{json} HTML5 Canvas - W3Schools [online]. 2019-07-29. [cit. 2019-07-29]. Dostupné z: \url{https://www.w3schools.com/html/html5\_canvas.asp}
